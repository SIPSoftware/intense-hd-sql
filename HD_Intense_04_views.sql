...
,decode(p.lp_dod,0,'',trim(to_char(p.nr_komp_doks,'0000000000'))||trim(to_char(p.nr_poz,'00000'))||trim(to_char(0,'00')))OSP_GlownaDoOspOrgKey
   ...
   ,p.nr_poz OSP_NrPoz
   ,decode(p.rodzaj_ceny,'zł/s','szt','m2') OSP_RodzajCeny
...
select
  ...
  ,0 TMG_Usuniety
  ,'Magayn surowc�w' TMG_Nazwa
  ,'Sur' TMG_Kod
from dual
union
select 'TOW','20181113',0,'Magazyn towar�w','Tow' from dual
union
select 'WYR','20181113',0,'Magazyn wyrob�w','Wyr' from dual
union
select 'CZY','20181113',0,'Magazyn czynno�ci','Czy' from dual
union
select 'POL','20181113',0,'Magazyn p�produkt�w','P�' from dual
union
...
select
  ...
  ,m.NAZ_MAG MAG_Nazwa
  ,m.nr_mag MAG_Kod
  ,decode(m.znacznik,'P�','POL','Sur','SUR','Tow','TOW','Czy','CZY','Wyr','WYR','OTHER') MAG_TmgOrgKey
